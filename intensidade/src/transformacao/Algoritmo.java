...
public class Algoritmo{
    ...
    public void powerLaw(){
        ...
        for(int i = 0; i < altura; i++) {
            for (int j = 0; j < largura; j++) {
                ...
                novaMatrizPixels[i][j] = suporteCores(this.matrizPixels[i][j], 0.2);
            }
        }
    ...
    private int suporteCores(int pixel, double y){
        Color cor = new Color(pixel, true);
        int novoPixel;
        Color novaCor = new Color((int) Math.pow(cor.getRed(), y*1), (int) Math.pow(cor.getGreen(), y*1), (int) Math.pow(cor.getBlue(), y*1), (int) Math.pow(cor.getAlpha(), y*1));
